        ...
        <div class="text-center well well-lg">
            <div class="container">
                <div class="input-group mb-2 mr-sm-2 mb-sm-0">
                    <input type="text" class="form-control" v-model="keyword" placeholder="Type Book name .. ex. Ender Game" :aria-invalid="validity">
                    <!-- <button id="btn" class="btn btn-outline-primary" @click="returnResults">Search!</button> -->
                    <button id="btn" class="btn btn-outline-primary" :disabled="validity" @click="returnResults">Search!</button>
                    <!-- <router-link :to="{ name: 'home'}"  :disabled="validity" @click="returnResults">    Search!</router-link> -->
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        beforeReturnResultFn () {
            ...
            //@Slate code - this fix is avoided by introducing router link in template.
            else  {
                // If button clicked reset the navigation to root,ie., '/'
                obj.$router.push({ name: 'home', path: '/' });
            }
            return true;
        },
        postReturnResultFn () {
        ...
        returnResults: function(pagetoFrmObj) {
            var obj = this;
            this.show = true;
            // Remove this element from component
            // this.page = (obj.$route.params.pd)?(obj.$route.params.pd):obj.page;
            this.page = (obj.page === 0)?(obj.page):pagetoFrmObj.to;
        ...
        returnPageResults (to, from) {
            console.log("return page results called !", to.path, from.path, this.keyword)
            let tofrm = {
                to: to.path,
                from: from.path,
            };
